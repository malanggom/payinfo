...
const submitForm = async (event) => {
  ...
  try {
    ...
    pjEventbus.SearchPjResultEvent.pjRefreshData(); // 이벤트 발생
...
// 모달 열기 핸들러
const pjOpenModalUpdateHandler = () => {
...
onMounted(async () => {
  pjEventbus.SearchPjResultEvent.add('pjOpenModalUpdate', pjOpenModalUpdateHandler); // 모달 열기 이벤트 등록
...
onUnmounted(() => {
  pjEventbus.SearchPjResultEvent.remove('pjOpenModalUpdate', pjOpenModalUpdateHandler); // 모달 열기 이벤트 제거
});
</script>
